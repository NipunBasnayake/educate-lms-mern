      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-4xl shadow-xl">
        <div className="flex justify-between items-start mb-6">
          <div>
            ...
            <p className="text-gray-600">{attempt.assessmentTitle || 'Untitled Assessment'}</p>
          </div>
          <div className="flex gap-2">
            {attempt.attemptNumber < (assessment?.attemptsAllowed || 3) && assessment?.status === 'active' && (
              <button
                onClick={onRetry}
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-8">
          ...
          <div className={`p-4 rounded-lg ${
            ...
            <div className="text-sm text-center">
              {attempt.passed ? 'Passed' : 'Failed'} (Requires {assessment?.passPercentage || 'N/A'}%)
            </div>
          </div>
        </div>
        <div className="mb-6">
              ...
              {attempt.questions.map((question, index) => (
                ...
                >
                  <div className="flex justify-between items-start">
                    <div>
                      <h4 className="font-medium text-gray-900">Q{index + 1}: {question.text || 'Untitled Question'}</h4>
                      <p className="text-sm text-gray-600 mt-1">
                        Your answer: <span className="font-mono bg-gray-100 px-1 rounded">
                          {attempt.answers[index] !== null
                            ? (question.options?.[attempt.answers[index]] || attempt.answers[index] || 'No answer')
                            : 'No answer'}
                        </span>
                      </p>
                    </div>
                  ...
                  {selectedQuestion === index && question.type === 'mcq' && (
                    <div className="mt-3 p-3 bg-gray-50 rounded-md">
                      <p className="text-sm font-medium text-gray-700">
                        Correct answer: {question.options?.[question.correctAnswer] || 'N/A'}
                      </p>
                    </div>
                  )}
                </div>
...
function QuizApp({ assessments, unitId }) {
  ...
  const getQuiz = async () => {
    try {
      const response = await getQuizByUnitId(unitId);
      ...
      const questions = response.data.map((quiz, index) => ({
        ...
        options: quiz.options[0] || [], // Options are nested in an array
      ...
      const newQuizQuestions = response.data.reduce((acc, quiz) => ({
        ...acc,
        [quiz._id]: {
          text: quiz.question || 'Untitled Question',
  ...
  const handleQuizComplete = (score, answers) => {
    const totalMarks = activeAssessment.totalMarks || activeAssessment.questions.reduce((sum, q) => sum + q.marks, 0);
    const scorePercentage = Math.round((score / totalMarks) * 100);
    const attemptNumber = attempts.filter(a => a.assessmentId === activeAssessment._id).length + 1;
    ...
    <div className="max-w-8xl mx-auto p-4 md:p-6 rounded-lg">
      ...
      {assessments.length === 0 ? (
        <div className="text-center py-10">
          ...
          <p className="mt-1 text-gray-500">
            {searchTerm || filter !== 'all' ? "Try adjusting your search or filter" : "No valid assessments available"}
          </p>
        </div>
      ) : (
        <div className="grid gap-4 md:gap-6">
          {assessments.map(assessment => {
            const assessmentAttempts = attempts.filter(a => a.assessmentId === assessment._id);
            const lastAttempt = assessmentAttempts[0];
            const attemptsLeft = (assessment.attemptsAllowed || 3) - assessmentAttempts.length;

            return (
              <div key={assessment._id} className="border p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow bg-white">
                <div className="flex flex-col md:flex-row md:justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-gray-600">{assessment.description || 'No description provided'}</p>
                        {assessment.dueDate && (
                  ...
                  <div className="flex flex-col items-end gap-2">
                    <div className="flex gap-4 text-sm">
                      <span className="text-gray-500">{assessment.questionsCount} Questions</span>
                    </div>
                    {lastAttempt && (
                    ...
                    <button
                      ...
                      disabled={attemptsLeft <= 0 || assessment.status === 'inactive'}
      ...
      {showInstructions && activeAssessment && (
        <QuizInstructions
          ...
          attemptNumber={attempts.filter(a => a.assessmentId === activeAssessment._id).length + 1}
          attemptsLeft={(activeAssessment.attemptsAllowed || 3) - attempts.filter(a => a.assessmentId === activeAssessment._id).length}
        />
      )}
      ...
      {showResults && currentAttempt && (
        <QuizResults
          ...
          assessment={assessments.find(a => a._id === currentAttempt.assessmentId)}
          onClose={() => setShowResults(false)}
          onRetry={() => {
            setShowResults(false);
            startAssessment(assessments.find(a => a._id === currentAttempt.assessmentId));
          }}
        />
      )}
    </div>
...
QuizApp.propTypes = {
  assessments: PropTypes.arrayOf(PropTypes.shape({
    ...
    quizList: PropTypes.arrayOf(PropTypes.string),
